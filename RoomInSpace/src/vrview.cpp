...
VRView::VRView(QWidget *parent) : QOpenGLWidget(parent),
   ...
   m_frames(0), m_mode(None), m_camera(new OrbitingCamera()), m_isDragging(false) {
...
void VRView::renderEye(vr::Hmd_Eye eye) {
   ...
   m_shader.bind();
   m_texture->bind(0);
   auto mat = viewProjection(eye);
   m_shader.setUniformValue("transform", helper.mat4x4ToQMatrix4x4(mat));
...
glm::mat4x4 VRView::vrMatrixToQt(const vr::HmdMatrix34_t& mat) {
   return glm::mat4x4(
...
glm::mat4x4 VRView::vrMatrixToQt(const vr::HmdMatrix44_t& mat) {
   return glm::mat4(
      mat.m[0][0], mat.m[0][1], mat.m[0][2], mat.m[0][3],
      mat.m[1][0], mat.m[1][1], mat.m[1][2], mat.m[1][3],
      mat.m[2][0], mat.m[2][1], mat.m[2][2], mat.m[2][3],
...
glm::mat4x4 VRView::viewProjection(vr::Hmd_Eye eye) {
   glm::mat4x4 s;
   s = 1000.0f * s;
   if (m_VRmode) {
      if (eye == vr::Eye_Left) {
      ...
      glm::mat4x4 proj = m_camera->getProjectionMatrix();
