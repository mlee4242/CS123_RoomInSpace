...
CMainApplication::CMainApplication(QWindow *parent)
   : QWindow(parent)
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMainApplication::CMainApplication(int argc, char *argv[], QWindow *parent)
   : QWindow(parent) // start of qopenglwindow
   , m_update_pending(false)
   , m_animating(false)
   , m_context(0)
   , m_device(0) // end of qopenglwindow
...
void CMainApplication::render(QPainter *painter) {
   Q_UNUSED(painter);
   RenderFrame();
}


void CMainApplication::renderLater() {
   if (!m_update_pending) {
...
bool CMainApplication::event(QEvent *event) {
   switch (event->type())
   {
...
void CMainApplication::exposeEvent(QExposeEvent *event) {
   Q_UNUSED(event);
...
void CMainApplication::renderNow() {
   if (!isExposed()) {
      return;
   }
...
void CMainApplication::setAnimating(bool animating) {
...
bool CMainApplication::BInit() {
   if (settings.renderMode == VR_MODE) {
...
//       m_SecondsTimer.start(1000, this);
   ...
   if ((settings.renderMode == VR_MODE) && !BInitCompositor()) {
...
void CMainApplication::RenderControllerAxes() {
   ...
   if ((settings.renderMode == COMPUTER_MODE) || m_pHMD->IsInputFocusCapturedByAnotherProcess()) {
      return;
   }
   ...
   for (vr::TrackedDeviceIndex_t unTrackedDevice = vr::k_unTrackedDeviceIndex_Hmd + 1; unTrackedDevice < vr::k_unMaxTrackedDeviceCount; ++unTrackedDevice) {
      if (m_pHMD && !m_pHMD->IsTrackedDeviceConnected(unTrackedDevice)) {
         continue;
      }

      if (m_pHMD && (m_pHMD->GetTrackedDeviceClass(unTrackedDevice) != vr::TrackedDeviceClass_Controller)) {
         continue;
      }
...
void CMainApplication::RenderScene(vr::Hmd_Eye nEye) {
   ...
   bool bIsInputCapturedByAnotherProcess = m_pHMD && m_pHMD->IsInputFocusCapturedByAnotherProcess();
