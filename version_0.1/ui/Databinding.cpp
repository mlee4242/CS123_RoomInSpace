...
IntBinding *IntBinding::bindSpinBox(QSpinBox *textbox, int& value) {
   // Bind the the textbox and the value together
   IntBinding *binding = new IntBinding(value);

   connect(textbox, SIGNAL(valueChanged(int)), binding, SLOT(intChanged(int)));
...
// resolution)
inline float mapValue(int i) {
   return static_cast<float>(i) * 0.01f;
}


inline int mapValue(float f) {
   return (int)ceilf(f * 100 - 0.5f);
}
